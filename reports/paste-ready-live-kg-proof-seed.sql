-- ============================================================================
-- Paste-Ready Live KG Proof Seed
-- Target: Supabase SQL Editor
-- Purpose: Optional proof seed for the next-generation KG layer.
--
-- Run this only after:
-- - public.canonical_entities exists
-- - public.curriculum_node_entities exists
-- - the live instance already has the KG foundation migration applied
--
-- Notes:
-- - This script is largely idempotent.
-- - It upserts canonical entities and external sources by slug.
-- - It uses conflict-safe inserts for bridge rows and source aliases.
-- ============================================================================
...
select
  count(*) as proof_curriculum_links
from public.curriculum_node_entities cne
join public.canonical_entities ce
  on ce.id = cne.canonical_entity_id
where ce.slug in (
  'intertrochanteric-fracture',
  'femoral-neck-fracture',
  'garden-classification',
...
select
  count(*) as proof_relationships
from public.canonical_relationships cr
join public.canonical_entities subj
  on subj.id = cr.subject_entity_id
join public.canonical_entities obj
  on obj.id = cr.object_entity_id
where subj.slug in ('intertrochanteric-fracture', 'femoral-neck-fracture', 'hip-hemiarthroplasty')
  or obj.slug in ('garden-classification', 'medial-femoral-circumflex-artery', 'avascular-necrosis', 'cephalomedullary-nail', 'femoral-neck-fracture');
