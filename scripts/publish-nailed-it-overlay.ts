/**
 * Publishes reviewed Nailed It enrichment as `Nailed_It` + `Nailed_It_Link`
 * field values, composed into the resource-field overlay the sync-v2 publisher
 * merges at build time.
 *
 ...
 * the new Nailed It fields per note_guid, writes a new combined overlay_key, and
 * optionally publishes it. Extra / Text / other resource fields are preserved.
 *
 * Usage:
 *   node --experimental-strip-types scripts/publish-nailed-it-overlay.ts --input <dir> --overlay-key <key>
 *   ... --apply
 *   ... --apply --confirm=PUBLISH_NAILED_IT_OVERLAY
 */
...
function loadNailedItFills(input: string): Map<string, { nailedIt: string; nailedItLink: string }> {
  ...
  const out = new Map<string, { nailedIt: string; nailedItLink: string }>();
  for (const file of use) {
    const packet = JSON.parse(readFileSync(file, "utf8")) as NailedItFillPacket;
    if (packet.schemaVersion !== NAILED_IT_ENRICHMENT_CONTRACT || packet.stage !== "fill") continue;
    for (const ov of overlaysFromFillPacket(packet)) {
      if (out.has(ov.stableGuid)) throw new Error(`duplicate_nailed_it_note_guid:${ov.stableGuid}`);
      out.set(ov.stableGuid, { nailedIt: ov.nailedIt, nailedItLink: ov.nailedItLink });
    }
  }
  return out;
}
...
async function main() {
  ...
  const publish = apply && confirm === "PUBLISH_NAILED_IT_OVERLAY";

  const fills = loadNailedItFills(input);
  const e = env();
  ...
  for (const [guid, fill] of fills) {
    ...
    if (existing) {
      existing.Nailed_It = fill.nailedIt;
      existing.Nailed_It_Link = fill.nailedItLink;
      updated += 1;
    } else {
      merged.set(guid, { Nailed_It: fill.nailedIt, Nailed_It_Link: fill.nailedItLink });
      added += 1;
    }
  }
  ...
  const summary = {
    ...
    nailedItFills: fills.size,
    nailedItNewNotes: added,
    nailedItUpdatedNotes: updated,
