...
} = require("../src/components/workspace/call/programcalltypes.ts");
const {
  generateCallSchedule,
} = require("../src/components/workspace/call/programcallautogenerator.ts");
const {
  debugCallGenerationMonth,
} = require("../src/lib/workspace/call/debugCallGeneration.ts");
...
async function main() {
  ...
  const [
    ...
    { data: ruleSetRow, error: ruleSetError },
  ] = await Promise.all([
    supabase
      .from("program_roster")
  ...
  if (!ruleSetRow || !ruleSetRow.id) throw new Error("No default rule set found");
  ...
  const effectiveRules = (rules || []).filter(
    (rule) => rule.rule_type !== "call_slot_definition"
  );
  ...
  const debugReport = debugCallGenerationMonth({
    ...
    generatedAssignments: {
      ...
      generationDebug:
        (generated.generationReport && generated.generationReport.generationDebug) || null,
    },
  });
