...
export async function POST(request: Request) {
  ...
  const parsed = OrthobulletsChatRequestSchema.safeParse(parsedBody);
  if (!parsed.success) {
    return NextResponse.json(
      { error: 'invalid_request', message: 'Invalid Orthobullets chat request.' },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    const response = parseOrthobulletsChatResponse({
    ...
    console.log('[brobot-orthobullets] chat_success', {
      requestId,
      userIdPrefix: auth.userId.slice(0, 8),
      questionId: resolvedContext.pageContext.questionId ?? null,
      latencyMs,
    ...
    console.error('[brobot-orthobullets] chat_failure', {
      requestId,
      userIdPrefix: auth.userId.slice(0, 8),
      questionId: resolvedContext.pageContext.questionId ?? null,
    ...
    return NextResponse.json(
      { error: 'api_failure', message: 'BroBot could not answer that follow-up.' },
      { status: 500 }
    );
  }
}
