...
function logBroBotCheckoutTrialDebug(
  label: string,
  details: Record<string, unknown>,
) {
  if (
    process.env.NODE_ENV !== 'production' ||
    process.env.BROBOT_CHECKOUT_DEBUG === 'true'
  ) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const checkoutSource =
      typeof body.checkoutSource === 'string' &&
      body.checkoutSource.trim().length > 0
    ...
    if (hasOngoingAccess) {
      ...
      try {
        ...
        ).createBillingPortalSession(user.id, portalReturnUrl);
        return NextResponse.json({
        ...
        return NextResponse.json({
          ...
          message:
            existingActive.status === 'past_due'
              ? 'Your payment method needs attention. Please manage it from your account.'
              : 'You already have an active BroBot subscription. Please manage it from your account.',
        });
      }
    }
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.log(
        '[billing/checkout] Using centralized billing redirect URLs:',
        {
