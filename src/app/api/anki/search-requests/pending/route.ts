...
// Housekeeping (stale-cancel + claim-requeue) is time-based, not result-based,
// so running it on every ~10s addon poll costs two writes per idle poll with
// nothing to show for it. Run it probabilistically instead; the 5-minute
// staleness windows make a few minutes of delay harmless.
const HOUSEKEEPING_PROBABILITY = 0.05;
...
export async function GET(request: Request) {
  ...
  if(Math.random()<HOUSEKEEPING_PROBABILITY){
