...
export async function GET(request:Request,{params}:{params:Promise<{id:string;sha256:string}>}){
 ...
 const{data:release}=await auth.supabase.from("anki_sync_v2_releases").select("status").eq("id",id).maybeSingle();
 if(!release||!["published","superseded"].includes(release.status))return NextResponse.json({error:"release unavailable"},{status:404});
 const{data:asset,error}=await auth.supabase.from("anki_sync_v2_release_media").select("*").eq("release_id",id).eq("content_sha256",sha256).maybeSingle();
