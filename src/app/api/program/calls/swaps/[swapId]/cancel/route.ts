...
import { cancelSwapRequest } from "@/lib/workspace/call-swaps/service";
import { cancelSwapRequestSchema } from "@/lib/workspace/call-swaps/validation";
...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    const parsed = cancelSwapRequestSchema.safeParse({
      requestId: swapId,
      cancelReason: body?.note,
    });
    ...
    const result = await cancelSwapRequest(
      createAdminClient(),
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error ? error.message : "Failed to cancel swap request",
      },
      { status: 500 }
    );
  }
