...
function BroBotExtensionLinkPageContent() {
  ...
  useEffect(() => {
    ...
    const ensureSignedIn = async () => {
      ...
      const redirectTo = `/brobot/extension/link?code=${encodeURIComponent(linkCode)}`;
  ...
  async function handleApprove() {
    if (!linkCode) {
      setErrorMessage('Missing link code. Return to the extension and start the link flow again.');
      setSuccessMessage('');
      return;
    }
    ...
    try {
      const response = await fetch('/api/brobot/extension/auth/approve-link', {
        method: 'POST',
        headers: {
      ...
      setSuccessMessage(
        'Extension approved. Return to Chrome and wait a few seconds for linking to finish.'
      );
    } catch (error) {
      setErrorMessage(
  ...
  return (
    <main className="min-h-screen bg-[#f7f5ef] px-6 py-16 text-[#1A1C2C]">
      <div className="mx-auto max-w-xl rounded-3xl border border-[#ddd6c8] bg-white p-8 shadow-sm">
        ...
        <h1 className="mt-3 text-3xl font-semibold">Approve this BroBot browser extension</h1>
        ...
        <p className="mt-2 text-sm leading-6 text-[#4f5464]">
          Approving this lets the Orthobullets side panel call your SnapOrtho BroBot quota and
          explanation APIs without relying on third-party site cookies.
        </p>
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          ...
          >
            {submitting ? 'Approving...' : 'Approve Extension'}
          </button>
          <Link
            href="/brobot"
...
export default function BroBotExtensionLinkPage() {
  return (
    <Suspense fallback={null}>
      <BroBotExtensionLinkPageContent />
    </Suspense>
  );
}
