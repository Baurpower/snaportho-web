...
async function getProgramCallAssignmentsForDates(
  ...
  if (error) {
    throw new Error(
      `Failed to load program calls for validation: ${error.message}`
    );
  }
...
async function getProgramCallAssignmentsForDateRange(
  ...
  if (error) {
    throw new Error(
      `Failed to load program calls for monthly validation: ${error.message}`
    );
  }
...
function getAcademicYearRangeForDates(dates: string[]) {
  const sorted = [...dates]
    .filter((date) => /^\d{4}-\d{2}-\d{2}$/.test(date))
    .sort();
  if (sorted.length === 0) return { start: null, end: null };
  const startDate = sorted[0];
  const endDate = sorted[sorted.length - 1];
  const academicStartYear = (date: string) => {
    const year = Number(date.slice(0, 4));
    const month = Number(date.slice(5, 7));
    return month >= 7 ? year : year - 1;
  };
  return {
    start: `${academicStartYear(startDate)}-07-01`,
    end: `${academicStartYear(endDate) + 1}-06-30`
  };
}
 ...
 */
function hasMonthlyCountRule(
  rules: Array<{ ruleType?: string | null; isEnabled?: boolean | null }>
): boolean {
  return rules.some(
    (rule) =>
...
export async function validateProgramCallMutationDraft(params: {
  ...
  const validationDateStart =
    monthRanges[0]?.start ?? uniqueTouchedDates[0] ?? null;
  const validationDateEnd =
    ...
    (uniqueTouchedDates.length > 0
      ...
      : null);
  const rotationHistoryRange = getAcademicYearRangeForDates(uniqueTouchedDates);
  ...
  const validationContext = await loadProgramCallValidationContext(
    ...
    null,
    {
      ...
      rotationHistoryStart: rotationHistoryRange.start,
      rotationHistoryEnd: rotationHistoryRange.end
    }
  );
  ...
  if (needsFullMonthExpansion && monthRanges.length > 0) {
    ...
    const perMonth = await Promise.all(
      monthRanges.map((range) =>
        getProgramCallAssignmentsForDateRange(
          params.programId,
          range.start,
          range.end
        )
  ...
  const upserts = (params.upserts ?? []).map((row, index) => ({
    ...
    id:
      row.id ??
      `draft-${index}-${row.callDate ?? "unknown"}-${row.callType ?? "unknown"}`
  }));
  ...
  const nextAssignments = [
    ...
    const residentId =
      ...
      (assignment.programMembershipId
        ? (residentIdByProgramMembershipId.get(
            assignment.programMembershipId
          ) ?? null)
        : null);
...
export async function resolveProgramRosterTargets(
  ...
  if (membershipIds.length > 0) {
    ...
    if (missingMembershipIds.length > 0) {
      ...
      if (error) {
        throw new Error(
          `Failed to validate membership-linked roster rows: ${error.message}`
        );
      }
...
export async function getCallAssignmentsForMembershipInRange(
  ...
  if (timedError) {
    throw new Error(
      `Failed to fetch timed calls in range: ${timedError.message}`
    );
  }
  ...
  if (dateOnlyError) {
    throw new Error(
      `Failed to fetch date-only calls in range: ${dateOnlyError.message}`
    );
  }
...
export async function createCallAssignmentsForMembership(
  ...
  const { data, error } = await supabase.from("call_assignments").insert(rows)
    .select(`
      id,
      program_id,
      program_membership_id,
...
export async function createProgramCallAssignments(
  ...
  if (error) {
    throw new Error(
      `Failed to create program call assignments: ${error.message}`
    );
  }
...
export async function getProgramCallStatsForMonth(
  ...
  if (rosterError) {
    throw new Error(
      `Failed to fetch program roster rows: ${rosterError.message}`
    );
  }
  ...
  if (callsError) {
    throw new Error(
      `Failed to fetch program call stats: ${callsError.message}`
    );
  }
