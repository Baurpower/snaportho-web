...
function rule(
  ...
  return { id, name: id, rule_type: rule_type as never, is_enabled: true, is_hard_rule, config };
}
...
const slotDefRules: ProgramRule[] = [
  rule("slot-backup", "call_slot_definition", { slotCallType: "Backup", slotFallbackPgyYears: [4], slotFallbackLabel: "Fallback: PGY-4 covering" }, false),
];
...
function evalAt(res: ResidentOption, slot: string, dateKey: string, assignments: Record<string, never> = empty) {
  const ctx = buildSchedulingContext({ residents, assignments });
  return evaluateSlot({ resident: res, slot, dateKey, ctx, policy, assignments, availabilityByResident: av });
}

// ── #4 + buddy first-month: intern eligibility by ortho-month index ──
ok("intern Buddy-eligible in 1st ortho month (Aug)", evalAt(intern, "Buddy", "2026-08-15").eligible);
ok("intern NOT Buddy-eligible in 2nd ortho month (Oct)", !evalAt(intern, "Buddy", "2026-10-17").eligible);
ok("intern NOT Primary-eligible in 1st ortho month (Aug)", !evalAt(intern, "Primary", "2026-08-04").eligible);
ok("intern Primary-eligible in 2nd ortho month (Oct) [#4]", evalAt(intern, "Primary", "2026-10-06").eligible);
ok("intern NOT Primary-eligible off-service (Sept)", !evalAt(intern, "Primary", "2026-09-08").eligible);

// Seniors' primary eligibility unchanged.
ok("PGY-2 still Primary-eligible", evalAt(pgy2, "Primary", "2026-08-04").eligible);
ok("PGY-5 still NOT Primary-eligible", !evalAt(pgy5, "Primary", "2026-08-04").eligible);
...
ok("PGY-5 Backup-eligible at preferred tier 0", backupPgy5.eligible && backupPgy5.tierPreference === 0);
ok("PGY-4 Backup-eligible at fallback tier 1", backupPgy4.eligible && backupPgy4.tierPreference === 1);
ok("PGY-4 Backup carries a fallback soft label", backupPgy4.warnings.some((w) => /fallback/i.test(w.message)));

// ── Buddy partner set {4,5}: presence + pairing ──
const ctxPgy5Primary = buildSchedulingContext({
  residents,
  assignments: { "2026-08-15": { primaryRosterId: "pgy5", backupRosterId: null, buddyRosterId: null } },
});
ok(
...
const buddyWithPgy5 = evaluateSlot({
  resident: intern, slot: "Buddy", dateKey: "2026-08-15", ctx: ctxPgy5Primary, policy,
  assignments: { "2026-08-15": { primaryRosterId: "pgy5", backupRosterId: null, buddyRosterId: null } },
...
const engine = makeEngineHelpers({
  rules: allRules, slotDefinitions: slotDefs, residents,
  assignments: { "2026-08-15": { primaryRosterId: "pgy4", backupRosterId: null, buddyRosterId: null } },
});
const buddyCands = engine.selectableResidentsForSlot("Buddy", "2026-08-15", residents).map((r) => r.residentId);
ok("buddy candidates on an Aug Sat = [intern]", JSON.stringify(buddyCands) === JSON.stringify(["intern"]));
const buddyCandsOct = engine.selectableResidentsForSlot("Buddy", "2026-10-17", residents).map((r) => r.residentId);
ok("buddy candidates in 2nd ortho month = [] (not eligible)", buddyCandsOct.length === 0);
