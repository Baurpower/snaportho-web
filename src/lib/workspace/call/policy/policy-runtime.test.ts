...
const buddyCandidates = engine
  ...
  .map((r) => r.residentId);
ok(
  "buddy candidates = [internGO]",
  JSON.stringify(buddyCandidates) === JSON.stringify(["internGO"])
);
ok(
  "buddy list excludes PGY-5 (the old bug)",
  !buddyCandidates.includes("pgy5")
);
ok(
  "buddy list excludes off-service intern",
  !buddyCandidates.includes("internOther")
);
...
const backupCandidates = engine
  ...
  .map((r) => r.residentId);
ok(
  "backup candidates = [pgy5]",
  JSON.stringify(backupCandidates) === JSON.stringify(["pgy5"])
);
...
const primaryCandidates = engine
  ...
  .map((r) => r.residentId);
ok(
  "primary includes pgy2 and pgy4",
  primaryCandidates.includes("pgy2") && primaryCandidates.includes("pgy4")
);
ok(
  "primary excludes interns and pgy5",
  !primaryCandidates.includes("internGO") && !primaryCandidates.includes("pgy5")
);

// ── presence ──
ok(
  "Buddy present on Sat under PGY-4 primary",
  engine.presence("Buddy", "2026-08-15").present
);
ok(
  "Backup present under PGY-2 primary",
  engine.presence("Backup", "2026-08-04").present
);
ok(
  "Buddy not present on a Tuesday",
  !engine.presence("Buddy", "2026-08-04").present
);

// ── isAllowed sanity ──
ok(
  "internGO isAllowed Buddy",
  engine.isAllowed(internGO, "Buddy", "2026-08-15")
);
ok("pgy5 NOT allowed Buddy", !engine.isAllowed(pgy5, "Buddy", "2026-08-15"));

const cappedEngine = makeEngineHelpers({
  rules,
  slotDefinitions: slotDefs,
  residents,
  assignments: {
...
ok(
  "manual Buddy picker blocks a third weekend at the monthly cap",
  !cappedEngine.isSelectable(internGO, "Buddy", "2026-08-15")
);
ok(
  "Buddy cap explains why the third weekend is unavailable",
  cappedEngine
    .evaluate(internGO, "Buddy", "2026-08-15")
    .blocks.some((block) => block.ruleType === "buddy_requirement")
);
