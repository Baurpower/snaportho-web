...
export type BroBotDeviceAuthMethod =
  | 'browser_session'
  | 'bearer_token'
  | 'device_token';
...
export type AuthenticatedDeviceLinkedRequest = {
  supabase: BrowserSupabaseClient | AdminSupabaseClient;
  userId: string;
  authMethod: BroBotDeviceAuthMethod;
  deviceLinkId: string | null;
  deviceTokenId: string | null;
};

type DeviceLinkedAuthOptions = {
...
function getDeviceToken(request: Request, headerName: string): string | null {
...
export function generateDeviceToken(prefix = 'snaportho_device'): string {
...
async function authenticateWithSupabaseUser(
  request: Request,
  options: DeviceLinkedAuthOptions
): Promise<
  | { success: true; context: AuthenticatedDeviceLinkedRequest }
  | { success: false; response: NextResponse | null }
> {
  if (!options.allowBrowserSession && !options.allowBearerToken) {
  ...
  if (bearerToken && options.allowBearerToken === false) {
    return {
      success: false,
      response: NextResponse.json(
        { error: 'Bearer authentication is not supported for this route.' },
        { status: 401 }
      ),
    };
  }

  if (!bearerToken && options.allowBrowserSession === false) {
    return { success: false, response: null };
  }
...
async function authenticateWithDeviceToken(
  ...
  | { success: false; response: NextResponse | null }
> {
  const rawDeviceToken = getDeviceToken(request, options.deviceTokenHeader);
  if (!rawDeviceToken) {
    return { success: false, response: null };
  }
...
export async function authenticateDeviceLinkedRequest(
  request: Request,
  options: DeviceLinkedAuthOptions
): Promise<AuthenticatedDeviceLinkedRequest | { response: NextResponse }> {
  const userAuth = await authenticateWithSupabaseUser(request, options);
  if (userAuth.success) return userAuth.context;
  if (userAuth.response) return { response: userAuth.response };

  const deviceAuth = await authenticateWithDeviceToken(request, options);
  if (deviceAuth.success) return deviceAuth.context;
  if (deviceAuth.response) return { response: deviceAuth.response };
...
> {
  ...
  return { success: true, supabase, link: data };
}
