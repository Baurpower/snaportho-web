...
function coerceBulletArray(value: unknown, maxItems: number, maxChars: number): string[] {
  return coerceStringArray(value, maxItems).map((item) => truncate(item, maxChars));
}

function coerceDifficulty(value: unknown): 'MS3' | 'PGY1' | 'PGY2+' {
  const raw = typeof value === 'string' ? value.toUpperCase() : '';
  if (raw.includes('MS')) return 'MS3';
  if (raw.includes('PGY1') || raw.includes('PGY-1')) return 'PGY1';
  return 'PGY2+';
}
 ...
 */
function salvageCurriculumPayload(
  parsed: Record<string, unknown>,
  emphasis: CurriculumExplainEmphasis
): CurriculumStudyPayload | null {
  const oneSentenceTakeaway =
    typeof parsed.oneSentenceTakeaway === 'string' ? truncate(parsed.oneSentenceTakeaway, 280) : '';
...
export function parseCurriculumStudyResponse(input: {
  ...
  if (salvaged) {
    const recoveredCurriculum = CurriculumStudyResponseSchema.safeParse({
      ...salvaged,
      responseKind: 'curriculum',
      explanationId: input.explanationId,
      usage: {
