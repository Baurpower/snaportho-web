...
export const CurriculumStudyResponseSchema = z.object({
  ...
  classifications: z.array(MustKnowGroupSchema).max(4).default([]),
  inThirtySeconds: z.array(z.string().trim().min(1).max(200)).min(1).max(6),
  mustKnow: z.array(MustKnowGroupSchema).max(8).default([]),
  clinicalPearls: z.array(z.string().trim().min(1).max(240)).max(12).default([]),
  commonMistakes: z.array(z.string().trim().min(1).max(240)).max(8).default([]),
  attendingQuestions: z.array(AttendingQuestionSchema).max(8).default([]),
  testableFacts: z.array(z.string().trim().min(1).max(240)).max(16).default([]),
  miniQuiz: z.array(MiniQuizItemSchema).max(6).default([]),
