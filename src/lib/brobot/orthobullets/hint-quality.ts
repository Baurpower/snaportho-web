import type { OrthobulletsHintRequest, OrthobulletsPageContext } from './types';
...
const CONTRAST_LANGUAGE =
  /\b(?:rather than|more than|less than|compared with|compared to|versus|while|whereas|stronger|weaker|distinguish|separate|combine|combination|relative)\b/i;
...
export function validateOrthobulletsHintDraft(input: {
  draft: HintDraft;
  hintLevel: 1 | 2 | 3;
  pageContext: Pick<OrthobulletsPageContext, 'answerChoices'>;
  priorHints: OrthobulletsHintRequest['priorHints'];
}) {
  const issues: string[] = [];
