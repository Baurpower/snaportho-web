...
const fixture = (name: string) =>
  JSON.parse(
    readFileSync(join(process.cwd(), 'fixtures/brobot-contract', name), 'utf8')
  );
...
assert.deepEqual(
  serializeLegacyResponse(internal),
  fixture('legacy-success.json')
);
assert.deepEqual(
  serializeWebResponseV2(internal),
  fixture('web-v2-success.json')
);
assert.equal(
  serializeLegacyResponse(internal).answer,
  serializeWebResponseV2(internal).answer
);
assert.equal('responseVersion' in serializeLegacyResponse(internal), false);
assert.deepEqual(
  Object.keys(serializeLegacyResponse(internal)).sort(),
  Object.keys(fixture('legacy-success.json')).sort()
);
for (const key of [
  'conversationId',
  'messageId',
  ...
  assert.ok(
    BROBOT_LEGACY_RESPONSE_KEYS.includes(key as never),
    `legacy key allowlist must include ${key}`
  );
}

const internalWithFutureFields = {
  ...internal,
  responseVersion: 999,
  citations: [{ title: 'web only' }]
};
const futureSafeLegacy = serializeLegacyResponse(
  internalWithFutureFields as BroBotChatInternalResult
);
assert.equal('responseVersion' in futureSafeLegacy, false);
assert.equal(
  'citations' in futureSafeLegacy,
  false,
  'new internal fields must not alter legacy decoding'
);
assert.equal(
  (
    serializeWebResponseV2(
...
assert.deepEqual(
  selectBroBotResponseContract(
    new Headers({ 'X-BroBot-Response-Version': '99' })
  ),
  {
    ok: false,
    requestedVersion: '99'
...
assert.deepEqual(
  selectBroBotResponseContract(
    new Headers({
      'X-BroBot-Response-Version': '1',
      'X-SnapOrtho-Client': 'ios'
    })
  ),
  {
    ok: true,
...
assert.deepEqual(
  selectBroBotResponseContract(
    new Headers({ 'X-BroBot-Response-Version': '  ' })
  ),
  {
    ok: false,
...
assert.deepEqual(clarification.clarifyingQuestions, [
...
assert.equal(
  branchSelection.nextLearningBranches?.[0]?.rankScore,
  42,
  'already-shipped legacy branch fields remain unchanged'
);
  ...
  nextLearningBranches: [
    {
      ...
      rankScore: 42,
      canonicalPrompt: 'Internal prompt',
      provenance: ['model'],
      safetyWarnings: []
    } as never
  ]
});
const serializedLegacyBranch = branchWithFutureFields
  .nextLearningBranches?.[0] as Record<string, unknown>;
...
assert.deepEqual(
  fixture('legacy-error.json'),
  fixture('web-v2-error.json'),
  'error JSON remains version-agnostic'
);

const withoutOptionalMetadata = { ...internal };
assert.deepEqual(
  serializeLegacyResponse(withoutOptionalMetadata),
  fixture('legacy-success.json')
);
...
assert.equal(
  shouldUseCasePrepV2Migration(
    'legacy',
    'What is the incision for carpal tunnel release?'
  ),
  false
);
assert.equal(
...
assert.equal(
  shouldUseCasePrepV2Migration(
    'legacy',
    'What is the incision for cubital tunnel release?'
  ),
  false
);
assert.equal(
