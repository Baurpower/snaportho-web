...
export function buildInitialNoteRelease(cards:V1ManifestCard[],version:string){
 ...
 const notes=[...groups.entries()].sort(([a],[b])=>a.localeCompare(b)).map(([noteGuid,rows])=>{
  ...
  const deckPaths=[...new Set(ordered.map(row=>row.deckPath))];
  if(deckPaths.length!==1)throw new Error(`cloze_sibling_deck_mismatch:${noteGuid}`);
  return{noteGuid,fields,governedTags,deckPath:deckPaths[0],expectedCardOrdinals:ordered.map(row=>row.cardOrdinal),contentChecksum:fieldChecksum,tagsChecksum:checksum(governedTags)};
 });
