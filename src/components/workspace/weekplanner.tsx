...
export function WeekPlannerPanel({
  ...
  const changedDates = useMemo(
    () => days.map((day) => day.date).filter((date) => drafts[date]?.selected && drafts[date]?.dirty),
    [days, drafts]
  );
  ...
  function assignDayToCurrentCategory(date: string) {
    ...
    setDrafts((prev) => {
      ...
      const alreadySelectedInSameCategory = current.selected && current.category === category;
      ...
      const currentTitleIsDefault = ["OR", "Clinic", "Custom"].includes(current.title);
      ...
      return {
        ...
        [date]: {
          ...
          selected: true,
          category,
          title: currentTitleIsDefault ? defaultTitleForCategory(category) : current.title,
          dirty: true,
        },
      };
    });
  ...
  async function handleSave() {
    try {
      ...
      setSuccessMessage(
        `Saved ${changedDates.length} change${changedDates.length === 1 ? "" : "s"}.`
      );
      ...
      setDrafts((prev) => {
        ...
        for (const date of Object.keys(next)) {
          ...
          next[date] = {
            ...next[date],
            dirty: false,
            loadedFromServer: next[date].selected ? true : next[date].loadedFromServer,
          };
        }
        return next;
      });
        ...
        <div>
          <div className="mb-3 flex items-center justify-between gap-3">
            ...
            <p className="text-xs text-slate-500">
              Loaded days can be edited immediately
            </p>
          </div>
          ...
          <p className="mt-3 text-xs text-slate-500">
            Single click assigns the current category. Double click jumps into editing that day.
          </p>
        </div>
            ...
            <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
              <div>
                ...
                <p className="mt-3 text-sm text-slate-500">
                  Switch between categories and edit loaded or newly added days in the same flow.
                </p>
              </div>
