...
as $$
  ...
  scored as (
    select d.*,
      (select count(*)::integer from unnest(d.terms) term where strpos(d.document, term) > 0) as hits,
      to_tsvector('english', d.document) as document_vector,
      to_tsquery('english', array_to_string(
        array(select quote_literal(term) from unnest(d.terms) term),
        ' | '
      )) as term_query
    from release_documents d
    where cardinality(d.terms) >= 1
  )
  select
    deck_release_id,
    release_version,
  ...
  from scored
  where hits >= case
    when cardinality(terms) = 1 then 1
    else greatest(2, ceil(cardinality(terms) * 0.5)::integer)
  end
  order by
...
revoke all on function public.search_latest_anki_deck_by_concept(text[], integer) from public, anon, authenticated;
grant execute on function public.search_latest_anki_deck_by_concept(text[], integer) to service_role;

comment on function public.search_latest_anki_deck_by_concept(text[], integer) is
  'Ranks version-pinned cards from only the newest published SnapOrtho deck release. Returns identities, never card bodies.';
