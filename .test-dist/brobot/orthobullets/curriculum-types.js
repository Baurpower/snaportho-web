...
exports.CurriculumStudyResponseSchema = zod_1.z.object({
    ...
    classifications: zod_1.z.array(MustKnowGroupSchema).max(4).default([]),
    inThirtySeconds: zod_1.z.array(zod_1.z.string().trim().min(1).max(200)).min(1).max(6),
    mustKnow: zod_1.z.array(MustKnowGroupSchema).max(8).default([]),
    clinicalPearls: zod_1.z.array(zod_1.z.string().trim().min(1).max(240)).max(12).default([]),
    commonMistakes: zod_1.z.array(zod_1.z.string().trim().min(1).max(240)).max(8).default([]),
    attendingQuestions: zod_1.z.array(AttendingQuestionSchema).max(8).default([]),
    testableFacts: zod_1.z.array(zod_1.z.string().trim().min(1).max(240)).max(16).default([]),
    miniQuiz: zod_1.z.array(MiniQuizItemSchema).max(6).default([]),
