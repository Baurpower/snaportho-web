...
exports.buildOrthobulletsExplainMessages = buildOrthobulletsExplainMessages;
exports.buildOrthobulletsChatMessages = buildOrthobulletsChatMessages;
exports.buildOrthobulletsHintMessages = buildOrthobulletsHintMessages;
function renderChoices(context) {
    if (!context.pageContext.answerChoices.length)
        return '(no answer choices detected)';
...
function renderDistribution(context) {
    if (!context.pageContext.percentDistribution.length)
        return '(not visible)';
...
- No markdown bullets or headers inside "answer".
- "suggestedPrompts" should contain 0-3 short, useful next follow-ups only when there is an obvious next step.`;
function buildOrthobulletsExplainMessages(context) {
    ...
    return [
        ...
        },
        {
            ...
            content: [
                ...
                `Linked concepts: ${context.pageContext.linkedConcepts.map((item) => item.label).join(', ') || '(none)'}`,
                `Image count: ${context.pageContext.images.length}`,
                kgNotes,
                `Extraction warnings: ${context.warnings.join(' | ') || '(none)'}`,
            ].join('\n\n'),
...
function renderKgNotes(context) {
...
function renderSourceReferences(context) {
...
function resolveQuestionImageUrls(context) {
...
function buildOrthobulletsHintMessages(input) {
    const ladderLabel = input.hintLevel === 1
    ...
    const userText = [
        ...
        `Question figures: ${imageUrls.length
