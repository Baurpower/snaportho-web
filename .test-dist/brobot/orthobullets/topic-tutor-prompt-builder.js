...
const ACTION_INSTRUCTIONS = {
    quiz_me: 'Requested action: "Quiz me from this page". Ask ONE new page-grounded question the learner has not already been asked in this conversation, following the tier ladder below.',
    find_answer: 'Requested action: "Ask me to find answers on the page". Pose a "Find the bullet that explains ___" retrieval question that points at a specific section/heading without revealing the answer.',
    explain_section: 'Requested action: "Explain this section". Pick the current or most recently discussed section and ask the learner to state its clinical significance BEFORE you confirm or expand on it — do not just explain it unprompted.',
    what_tested: 'Requested action: "What would be tested?". Ask the learner what detail on this page is most likely to appear on OITE/boards, and let them answer first before you confirm.',
    attending_question: 'Requested action: "What would an attending ask?". Ask a pimp-style question a PGY2+ attending would ask right after reading this page.',
    explain_images: 'Requested action: "Explain the images". Reference the provided image alt/caption metadata and ask what diagnosis or principle it illustrates before confirming. If no images were extracted, say so plainly and offer another action instead of inventing image content.',
    board_traps: 'Requested action: "Show board traps". Ask the learner to identify the trap/pitfall implied by this page before you reveal it.',
    save_missed: 'Requested action: "Save missed concepts". Do not ask a new question. Briefly name the concept(s) from the recent conversation the learner struggled with (conceptStatus="missed", conceptTag set), confirm it is saved for review, and set message to a short confirmation.',
};
...
function renderCounts(context) {
    const parts = [];
    if (context.pageContext.questionCount != null)
        parts.push(`${context.pageContext.questionCount} questions`);
    if (context.pageContext.cardCount != null)
        parts.push(`${context.pageContext.cardCount} cards`);
    if (context.pageContext.videoCount != null)
        parts.push(`${context.pageContext.videoCount} videos`);
...
function buildTopicTutorMessages(input) {
