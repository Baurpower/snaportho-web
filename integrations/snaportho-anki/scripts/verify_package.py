...
with zipfile.ZipFile(package)as archive:
    ...
    manifest=json.loads(archive.read("manifest.json"));config=json.loads(archive.read("config.json"));assert manifest["version"]==VERSION;assert config["environment"]in{"local","staging","production"}
