...
class CardWorkspace:
    def __init__(self,parent,runtime,card,gateway,source_surface="reviewer",can_previous=False,can_next=False):
        ...
        subtitle=QLabel("Review card content, learning resources, and knowledge-graph mappings without exposing personal study data.");subtitle.setObjectName("workspaceSubtitle");subtitle.setWordWrap(True);root.addWidget(subtitle)
        ...
        actions=QHBoxLayout();previous=QPushButton("← Previous");previous.setEnabled(can_previous);previous.clicked.connect(lambda:self.navigate("previous"));next_button=QPushButton("Next →");next_button.setEnabled(can_next);next_button.clicked.connect(lambda:self.navigate("next"));save=QPushButton("Save draft");save.clicked.connect(self.save_draft);discard=QPushButton("Discard");discard.clicked.connect(self.discard_draft);preview=QPushButton("Preview");preview.clicked.connect(self.preview_upload);submit=QPushButton("Submit proposal");submit.setObjectName("primaryAction");submit.clicked.connect(self.submit);close=QPushButton("Close");close.clicked.connect(self.dialog.reject);actions.addWidget(previous);actions.addWidget(next_button);actions.addStretch(1);actions.addWidget(discard);actions.addWidget(save);actions.addWidget(preview);actions.addWidget(submit);actions.addWidget(close);root.addLayout(actions);self._resolve()
    ...
    def submit(self):
        ...
        def done(future):
            ...
            except Exception as error:self.runtime.store.mark_workspace(identity["noteGuid"],identity["cardOrdinal"],self._draft_version(),"conflict"if getattr(error,"conflict_type",None)else"draft");self.status.setText(f"Submission failed — {describe(error)}")
